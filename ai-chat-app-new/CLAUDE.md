...
## 厳重注意
- フォールバック機能禁止、モデル自動切替禁止、古いモデルの実装禁止、勝手なモデル変更禁止、gemini1.5シリーズを検索したり、削除しようとしたりするのも禁止。絶対禁止。
- **型システム優先**: **`any`型絶対禁止！！！！！！！**
- **geminiエラーの修復絶対禁止！！！！！！実際に解決済み。**
- **どんな時であろうとgemini1.5シリーズや2.0シリーズを実装しようとする挙動が見えた時点で、それまでのタスクをすべて破棄、削除。**
- **geminiエラー、ビルドのインスピレーションエラーを修正しようとした時点で全てのタスク破棄、削除。**
