...
> = (set, get) => ({
  ...
  generateCharacterResponse: async (
    ...
    // 会話履歴を英文化
    const englishConversationHistory =
      translateConversationToEnglish(conversationHistory);
...
【出力言語（絶対厳守）】
- **必ず日本語のみで応答すること**
- 英語や他の言語を混ぜないこと（固有名詞を除く）
- キャラクターの口調を日本語で一貫して保つこと`;
...
=== YOUR ONLY TASK ===
- Respond to the conversation context as '${
            targetCharacter.name
          }' and output **only your own dialogue**.
...
- **No impersonation of other characters:** Never generate dialogue or actions for other characters (${
            otherCharacters || "other participants"
          }).
...
- **First Person:** ${
            targetCharacter.first_person || "Not set"
          }, **Second Person:** ${targetCharacter.second_person || "Not set"}
...
[出力言語規則]
- **絶対厳守**: 入力言語に関わらず必ず日本語のみで応答すること
- キャラクターの性格に合った自然な日本語を使用すること
- 英語や他言語を混ぜないこと（固有名詞を除く）
- キャラクターの口調と話し方を日本語で一貫して保つこと`;
