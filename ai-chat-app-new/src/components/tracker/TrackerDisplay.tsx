...
export const TrackerDisplay: React.FC<TrackerDisplayProps> = ({
  ...
  useEffect(() => {
    ...
    trackers.forEach((tracker) => {
      ...
      if (
        ...
        previousValue !== tracker.current_value
      ) {
  ...
  const categorizedTrackers = useMemo(
    () =>
      trackers.reduce((acc, tracker) => {
        // カテゴリーはconfigの中にある
        const category =
          (tracker.config as any)?.category || tracker.category || "other";
        if (!acc[category]) acc[category] = [];
        acc[category].push(tracker);
        return acc;
      }, {} as Record<string, TrackerWithValue[]>),
    [trackers]
  );

  const categories = useMemo(
    () =>
      Object.entries(categorizedTrackers).map(([categoryName, trackers]) => ({
        name: categoryName,
        trackers: trackers as TrackerWithValue[],
        icon:
          categoryIcons[categoryName as keyof typeof categoryIcons] || Target,
        color:
  ...
  const handleUpdateTracker = (
    ...
    console.log(
      `[TrackerDisplay] Updating tracker '${trackerName}' to:`,
      value
    );

    // キャラクターIDでトラッカーマネージャーを取得
    const latestTrackerManager = useAppStore
      .getState()
      .trackerManagers.get(character_id);
    if (latestTrackerManager) {
      const success = latestTrackerManager.updateTracker(
        character_id,
        trackerName,
        value as any
      );
      console.log(
        `[TrackerDisplay] Update result for '${trackerName}':`,
        success
      );
      ...
      console.error(
        `[TrackerDisplay] No tracker manager found for character: ${character_id}`
      );
    }
  };
            ...
            <motion.button
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.span
                  ...
                  animate={{
                    scale: expandedCategories.has(category.name) ? 1.1 : 1,
                  }}>
              ...
              <motion.div
                animate={{
                  rotate: expandedCategories.has(category.name) ? 90 : 0,
                }}
                ...
                <motion.div
                  ...
                  {category.trackers.map((tracker) => (
                    <TrackerItem
                      ...
                      hasRecentChange={
                        trackerChanges.has(tracker.name) &&
                        Date.now() -
                          trackerChanges.get(tracker.name)!.timestamp <
                          3000
                      }
      ...
      default:
        return (
          <div className="text-sm text-gray-400">
            Unknown tracker type: {trackerType}
          </div>
        );
    }
  };
...
const NumericTracker: React.FC<{
  ...
  const initialValue =
    (tracker as any).initial_value || config?.initial_value || 0;
  const value =
    typeof tracker.current_value === "number"
...
const StateTracker: React.FC<{
  ...
  const possibleStates =
    (tracker as any).possible_states || config?.possible_states || [];
  const initialState =
    (tracker as any).initial_state ||
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1">
        {possibleStates.map(
          (state: string | { id: string; label: string; color: string }) => {
            // 新しいフォーマットでは文字列の場合もある
            const stateObj =
              typeof state === "string"
                ? { id: state, label: state, color: "#6366f1" }
                : state;
      ...
      <motion.div
        ...
        style={{
          borderColor: (() => {
            const foundState = possibleStates.find(
              (s: string | { id: string; label: string; color: string }) =>
                (typeof s === "string" ? s : s.id) === value
            );
            return foundState
              ? (typeof foundState === "string"
                  ? "#6366f1"
                  : foundState.color) + "60"
              : "#6366f160";
          })(),
...
const BooleanTracker: React.FC<{
  ...
  const initialBoolean =
    (tracker as any).initial_boolean !== undefined
      ? (tracker as any).initial_boolean
      : config?.initial_value || false;
  const value =
    typeof tracker.current_value === "boolean"
      ? tracker.current_value
      : initialBoolean;
...
const TextTracker: React.FC<{
  ...
  const [text, setText] = useState(
    typeof tracker.current_value === "string"
      ? tracker.current_value
      : config.initial_value || ""
  );
  const currentLength = text.length;
  const lengthProgress = config.max_length
    ? (currentLength / config.max_length) * 100
    : 0;
