...
> = ({ message, isLatest = false, isGroupChat = false }) => {
  ...
  const stages = useMemo(
    () => progressiveData.stages || { reflex: undefined, context: undefined, intelligence: undefined },
    [progressiveData.stages]
  ) as ProgressiveMessage['stages'];
  const currentStage = progressiveData.currentStage || "reflex";
  const ui: ProgressiveMessage['ui'] = (progressiveData.ui as ProgressiveMessage['ui'] | undefined) || {
    showIndicator: true,
    isUpdating: false,
    glowIntensity: "none",
  ...
  const getCurrentStageContent = useCallback(() => {
    if (selectedStage && selectedStage in stages) {
      const stage = stages[selectedStage as keyof typeof stages];
      if (stage?.content) {
