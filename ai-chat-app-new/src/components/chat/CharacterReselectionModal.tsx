...
export const CharacterReselectionModal: React.FC<
  CharacterReselectionModalProps
> = ({ session, isOpen, onClose }) => {
  const { characters, updateSessionCharacters, groupSessions } = useAppStore();
  ...
  const availableCharacters = useMemo(() => {
    ...
    return list.filter((char: Character) => char?.is_active);
  }, [characters]);
  ...
  const changePreview = useMemo((): CharacterChangePreview => {
    ...
    const added = availableCharacters.filter(
      (char) => newIds.has(char.id) && !currentIds.has(char.id)
    );
    const removed = session.characters.filter((char) => !newIds.has(char.id));
    const kept = session.characters.filter((char) => newIds.has(char.id));

    // 削除されるキャラクターのメッセージ数を計算
    const removedCharacterIds = new Set(removed.map((c) => c.id));
    const affectedMessages = session.messages.filter(
      (msg) => msg.character_id && removedCharacterIds.has(msg.character_id)
    ).length;
    ...
    if (removed.length > 0) {
      warnings.push(
        `${removed.length}人のキャラクターが削除され、${affectedMessages}件のメッセージが影響を受けます`
      );
    }
  ...
  const handleSave = () => {
    ...
    try {
      ...
      if (typeof updateSessionCharacters === "function") {
        ...
        const storeState = useAppStore.getState?.() || {} as ReturnType<typeof useAppStore.getState>;
        if (typeof storeState.updateGroupMembers === "function") {
          ...
          console.error("No session update function found in store:", {
    ...
    if (changePreview.added.length > 0 || changePreview.removed.length > 0) {
      ...
      if (changePreview.added.length > 0) {
        changeMessage.push(
          `${changePreview.added.map((c) => c.name).join("、")}が参加しました`
        );
      }
      if (changePreview.removed.length > 0) {
        changeMessage.push(
          `${changePreview.removed.map((c) => c.name).join("、")}が退出しました`
        );
      }
              ...
              <div className="px-6 pb-6 flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-400/20 scrollbar-track-slate-800">
                <div className="grid grid-cols-1 gap-3">
                  {availableCharacters.map((character) => {
                    ...
                    const wasOriginalMember = session.character_ids.includes(
                      character.id
                    );
                ...
                {changePreview.warnings.length > 0 && (
                  <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 mb-4">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <h5 className="text-sm font-medium text-yellow-300 mb-1">
                          注意事項
                        </h5>
                ...
                {changePreview.affectedMessages > 0 && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 mb-4">
                    <p className="text-sm text-red-300">
                      {changePreview.affectedMessages}
                      件のメッセージが削除されるキャラクターによるものです。
