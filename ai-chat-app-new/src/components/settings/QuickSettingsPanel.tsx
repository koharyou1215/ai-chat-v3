...
export const QuickSettingsPanel: React.FC<QuickSettingsPanelProps> = ({ isOpen, onClose }) => {
  ...
  // 🔧 FIX: Gemini直接APIモデルのみを対象（OpenRouter経由のGeminiは除外）
  const isGeminiDirectModel = apiConfig.model?.startsWith('gemini-') ?? false;
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <ToggleSwitch
            ...
            onChange={(checked) => {
              console.log('💬 [QuickSettings] 吹き出しぼかし効果 onChange:', checked);
              updateEffectSettings({ bubbleBlur: checked });
            }}
            icon="💬"
            description={effectSettings.bubbleBlur ? "吹き出し背景をぼかします" : "無効"}
          />

          {/* 🆕 Phase 2: ぼかし強度スライダー（簡易版・条件付き表示） */}
          {effectSettings.bubbleBlur && (
            <div className="ml-6 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-400">ぼかし強度</span>
                <span className="text-xs text-purple-400 font-medium">
                  {effectSettings.bubbleBlurIntensity || 8}px
                </span>
              </div>
              <input
                type="range"
                ...
                max="20"
                step="2"
                value={effectSettings.bubbleBlurIntensity || 8}
                onChange={(e) => {
                  const value = parseInt(e.target.value);
                  console.log('💬 [QuickSettings] ぼかし強度 onChange:', value);
                  updateEffectSettings({ bubbleBlurIntensity: value });
                }}
                className="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer"
                style={{
                  background: `linear-gradient(to right, #8b5cf6 0%, #8b5cf6 ${((effectSettings.bubbleBlurIntensity || 8) / 20) * 100}%, #4b5563 ${((effectSettings.bubbleBlurIntensity || 8) / 20) * 100}%, #4b5563 100%)`,
                }}
              />
            </div>
          )}
