...
export class ConversationManager {
  ...
  private estimateTokens(context: ConversationContext): number {
    ...
    totalChars += context.recent_messages.reduce(
      (sum: number, m: UnifiedMessage) => sum + m.content.length,
      0
    );
  ...
  private async getRelevantMemoryCards(
    ...
    try {
      ...
      const relevantCards = cards
        .map((card) => {
          const relevanceScore = this.calculateSmartMemoryCardRelevance(
            card,
            userInput,
            character
          );
        ...
        .sort((a, b) => {
          ...
          if (
            Math.abs(a.card.importance.score - b.card.importance.score) > 0.1
          ) {
