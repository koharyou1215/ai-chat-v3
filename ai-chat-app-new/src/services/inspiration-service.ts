...
export class InspirationService {
  ...
  private buildReplySuggestionPrompt(
    ...
    return `あなたは${speaker}({{user}}男側)として、${target}({{char}}女側)への返信を4つの異なるアプローチで生成してください。
...
- 説明や見出しは不要、返信文のみを出力`;
  ...
  private buildEnhancementPrompt(
    ...
    // より厳密な出力指示を付与して口調崩れ・途中切れを防ぐ
    return `あなたは表現力増強師です。
以下の"${inputText}"を、元の一人称・性別・口調を絶対に変更せずに、{{user}}らしい口調とキャラクターを保持したまま、元の意味を損なわずに自然で完結した文章に強化してください。

重要な指示（必ず遵守）:
- 元の一人称（例: 私、僕、俺）や性別的な口調を変更しないこと。決して女口調に変えない。
- 口調、敬体/常体、語尾の傾向を維持すること。
