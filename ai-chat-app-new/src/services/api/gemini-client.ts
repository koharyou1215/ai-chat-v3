...
export class GeminiClient {
        ...
        try {
          ...
          if (
            ...
            const quotaError = new Error(
              "Gemini APIの使用制限に達しました。約1分後に再試行してください。"
            );
          ...
          if (
            ...
            errorMessage.includes("is not a valid model")
          ) {
            console.error(
              `❌ モデル ${this.model} が見つかりません。gemini-2.5-flash、gemini-2.5-flash-light、またはgemini-2.5-proを使用してください。`
            );
            throw new Error(
              `無効なGeminiモデル: ${this.model}。gemini-2.5-flash、gemini-2.5-flash-light、またはgemini-2.5-proのいずれかを使用してください。`
            );
          }
        } catch (parseError) {
  ...
  setModel(model: string) {
    ...
    if (cleanModel.endsWith("-8b")) {
      console.warn(
        `⚠️ 不正なモデルサフィックス '-8b' を除去: ${cleanModel} → ${cleanModel.replace(
          "-8b",
          ""
        )}`
      );
    ...
    if (!validModels.includes(cleanModel)) {
      console.error(
        `❌ 無効なGeminiモデル: ${cleanModel}. デフォルトのgemini-2.5-flashを使用します`
      );
  ...
  getAvailableModels(): string[] {
    // 3つのモデルのみをサポート
    return ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-2.5-flash-light"];
  }
...
// Singleton getter for lazy initialization to avoid top-level side effects during build
let singletonGeminiClient: GeminiClient | null = null;

export function getGeminiClient(): GeminiClient {
  if (!singletonGeminiClient) {
    singletonGeminiClient = new GeminiClient();
  }
  return singletonGeminiClient;
}
