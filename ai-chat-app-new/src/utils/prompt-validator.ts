...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  (window as unknown as Record<string, unknown>).validateCurrentPrompt = (prompt: string, characterName: string) => {
    const result = promptValidator.validatePrompt(prompt, characterName);
    // console.log(promptValidator.generateReport(result)); // 無効化
    return result;
  };

  (window as unknown as Record<string, unknown>).checkResponse = (response: string, characterName: string) => {
