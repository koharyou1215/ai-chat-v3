...
export class PromptValidator {
  ...
  validatePrompt(
    ...
    // 3. キャラクター一貫性指示の確認
    if (
      !prompt.includes("キャラクター一貫性") &&
      !prompt.includes("性格・口調")
    ) {
  ...
  checkResponseForMeta(
    ...
    metaPatterns.forEach((pattern, index) => {
      if (pattern.test(response)) {
        switch (index) {
          case 0:
            metaIndicators.push("AI言及");
            break;
          case 1:
            metaIndicators.push("システム言及");
            break;
          case 2:
            metaIndicators.push("プログラム言及");
            break;
          case 3:
            metaIndicators.push("設定言及");
            break;
          case 4:
            metaIndicators.push("キャラクター分析");
            break;
          case 5:
            metaIndicators.push("物語メタ言及");
            break;
          case 6:
            metaIndicators.push("作品メタ言及");
            break;
          case 7:
            metaIndicators.push("未来の行方言及");
            break;
          case 8:
            metaIndicators.push("展開予測");
            break;
          case 9:
            metaIndicators.push("展開議論");
            break;
          case 10:
            metaIndicators.push("シナリオ言及");
            break;
          case 11:
            metaIndicators.push("ストーリー言及");
            break;
        }
      }
    });
    ...
    const severity =
      metaIndicators.length >= 3
        ? "high"
        : metaIndicators.length >= 1
        ? "medium"
        : "low";
   ...
   */
  generateReport(
    promptResult: PromptValidationResult,
    responseResult?: {
      hasMeta: boolean;
      metaIndicators: string[];
      severity: "low" | "medium" | "high";
    ...
    // スコアと推奨
    const scoreEmoji =
      promptResult.score >= 80 ? "✅" : promptResult.score >= 60 ? "⚠️" : "❌";
...
if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
  (window as unknown as Record<string, unknown>).validateCurrentPrompt = (
    prompt: string,
    characterName: string
  ) => {
  ...
  (window as unknown as Record<string, unknown>).checkResponse = (
    response: string,
    characterName: string
  ) => {
    const result = promptValidator.checkResponseForMeta(
      response,
      characterName
    );
